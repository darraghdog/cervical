...
sub = sub1
for(var in names(sub)[2:4]) sub[[var]] = (.2/3)* (sub1[[var]] + sub2[[var]] + sub3[[var]]) + 0.1 * (sub4[[var]]+ sub5[[var]]+ sub6[[var]]) + 0.25* (subdm1[[var]] + subdm2[[var]])
...
write.csv(sub, "remove_addtl_10x-leakv5.csv", row.names = F)
