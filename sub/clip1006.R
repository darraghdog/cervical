...
########################################
sub <- fread("small_large_crop_and_full_small_alexnet_test_or_cropped_tested_ensemble_15.csv")
...
for(var in names(sub)[2:4]) sub[[var]] = 0.25 * (sub4[[var]]+ sub5[[var]]+ sub6[[var]]+ sub7[[var]]) + 0.5*(sub8[[var]]) # + 0.15* (subdm1[[var]] + subdm2[[var]])
...
write.csv(sub, "just_darragh_4_subs.csv", row.names = F)
